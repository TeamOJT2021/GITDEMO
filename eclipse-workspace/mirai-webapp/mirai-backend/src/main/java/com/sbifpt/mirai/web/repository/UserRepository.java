...
//	User getAllUserByUserIdAndPerId(long userId, long perId);
	@Query("SELECT u FROM User u JOIN u.roles r JOIN r.permissions p  WHERE r.roleId = ?1 AND p.permissionId =?2")
	List<User> getAllUserByRoleIdAndPerId(long roleId, long perId);
